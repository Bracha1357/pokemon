...
let loadMorePokemon = async () => {
    try{
        ...
        console.log('pokemon on state after load pokemon', pokemon)
        setNextPage(data.next);
    }catch (err){
        console.log(err);
...
return (
    ...
    <div className='row'>
        {pokemon.map((poke) =>{
            ...
            return(
                <div key={poke.name} className = 'card col-md-2 mb-4 mx-3 style_color' >
                    <Link to={`/pokemon/${pokemonId}`} className='color_text'>
